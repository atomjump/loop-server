<?php
	...
	if(($argc > 1)&&($argv[1] != 'all')) {
		...
		upload_to_all(getcwd() . "/images/im/" . $argv[1], $argv[1], null, "/images/im/");
	
	} else {
	
		if(isset($_REQUEST['image'])&&($_REQUEST['code'] == 'sdfsdfgew3345')) {
			global $local_server_path;
			upload_to_all($local_server_path . "images/im/" . $_REQUEST['image'], $_REQUEST['image'], null, "/images/im/");
			...
			if(($argc > 1)&&($argv[1] == 'all')) {
				...
				foreach ($dir as $fileinfo) {
					if (!$fileinfo->isDot()) {
						...
						upload_to_all($path . $filen, $filen, $specific, "/images/im/");
